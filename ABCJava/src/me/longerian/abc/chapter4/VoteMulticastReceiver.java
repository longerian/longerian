...
public class VoteMulticastReceiver {
	...
	public static void main(String[] args) throws IOException {
		InetAddress address = InetAddress.getByName("224.0.0.1"); // Multicast
																// address
		if (!address.isMulticastAddress()) { // AldTest if multicast address
			throw new IllegalArgumentException("Not a multicast address");
		}

		int port = 9001; // Multicast port
		...
		VoteMsg vote = coder.fromWire(Arrays.copyOfRange(packet.getData(), 0,
				packet.getLength()));

...
public class TradesBoughtGetParser extends TaobaoParser {

	@Override
	public TaobaoResponse parse(String strResponse) {
		TradesBoughtGetResponse resp = new TradesBoughtGetResponse();
		if(strResponse != null) {
			LogUtil.d(this.getClass().getName(), strResponse);
			try {
				JSONObject jsonResponse = new JSONObject(strResponse).getJSONObject("trades_bought_get_response");
				JSONObject jsonObjTrades = jsonResponse.getJSONObject("trades");
				JSONArray jsonArrTrades = jsonObjTrades.getJSONArray("trade");
				ArrayList<Trade> trades = new ArrayList<Trade>();
				int length = jsonArrTrades.length();
				for(int i = 0; i < length; i++) {
					JSONObject jsonTrade = jsonArrTrades.getJSONObject(i);
	...
	private Trade parseTradeFromJson(JSONObject json) throws JSONException, ParseException {
		...
		trade.setReceiverAddress(json.getString("receiver_address"));
		...
		trade.setReceiverName(json.getString("receiver_name"));
		trade.setSellerNick(json.getString("seller_nick"));
		trade.setSellerRate(json.getBoolean("seller_rate"));

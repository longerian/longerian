...
public class ImageLoaderWithScale implements Loadable {

	private HashMap<String, SoftReference<Bitmap>> mImageCache;
	
	public ImageLoaderWithScale() {
	...
	public Bitmap loadBitmapImage(String path) {
		...
		options.inSampleSize = Util.computeSampleSize(options, 800, (int) (1 * 1024 * 1024));
		Log.d("OomDemo", "inSampleSize: " + options.inSampleSize);
		options.inJustDecodeBounds = false;
		options.inDither = false;
		options.inPreferredConfig = Bitmap.Config.ARGB_8888;
		Bitmap bitmap = BitmapFactory.decodeFile(path, options);
		Log.d("OomDemo", "new size: " + bitmap.getRowBytes());
